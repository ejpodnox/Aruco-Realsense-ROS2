...
## New Feature: Z-Axis Rotation Tracking
We have added a new node `aruco_z_rotation_node` that specifically tracks the Yaw (Z-axis rotation) of a planar marker. This is useful for dial or knob rotation tracking.

**Key Features:**
- **Accumulated Rotation:** Tracks total rotation beyond 360 degrees (e.g., multiple turns).
- **Visualization:** Draws axis and rotation values directly on the image stream.
- **Data Logging:** Records timestamp, marker ID, current yaw, and accumulated yaw to a CSV file.
- **Robustness:** Handles occlusion and frame wrapping (359° -> 0°).
...
## Running the Rotation Tracker

To use the rotation tracking feature with an attached RealSense camera:
...
2. **Launch the tracker:**
   This one command launches the RealSense driver, the tracker node, and RViz visualization.
   ```bash
   ros2 launch aruco_z_rotation aruco_with_realsense.launch.py marker_id:=0 marker_size:=0.03
   ```
   
   **Arguments:**
   - `marker_id`: ID of the marker to track (default: 25).
   - `marker_size`: Physical size of the marker in meters (default: 0.04).
   - `dictionary_id`: ArUco dictionary (default: DICT_5X5_100). options: DICT_ORIGINAL, DICT_4X4_50, etc.
   - `csv_output`: File path to save rotation data (default: rotation_data.csv).
   - `show_window`: Set to `true` to see the OpenCV debug window (default: true).

The system will display a window showing the camera feed with axis overlays and the total accumulated rotation. Data is automatically saved to `rotation_data.csv`.

## Using Custom Cameras
If you are not using a RealSense camera, you can start your camera driver separately and run just the node:
```bash
ros2 run aruco_z_rotation z_rotation_node --ros-args -p camera_topic:=/my_camera/image_raw -p camera_info_topic:=/my_camera/camera_info
```
